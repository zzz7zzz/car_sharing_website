...
$(document).ready(function() {
	var storageFields = ["locFrom_rider", "locTo_rider", "datepicker_rider", "startTimeHour_rider", 
    					"startTimeMin_rider", "datepicker_rider"];
	...
	$("#datepicker_rider").datepicker();
	
	 $("#submit_rider").click(function()
  	{
  	    var day_in_unix = $("#datepicker_rider").datepicker("getDate").getTime()/1000;//it works
  		var start_hour_in_unix = $("#startTimeHour_rider").val();
  		var start_min_in_unix =  $("#startTimeMin_rider").val();
  		var startTime = day_in_unix + (start_hour_in_unix)*3600 + (start_min_in_unix)*60; 
   		$.ajax(
      	{
            url: "app/insert_rider_request.php",
            type: "POST",
        	data:
        	{
                locFrom: $("#locFrom_rider").val().toUpperCase(),
                locTo: $("#locTo_rider").val().toUpperCase(),
                timeFrom: startTime,
                date: $("#datepicker_rider").val(),
        	},
            dataType: "text",
...
<div class = "fixed-width-centered">
		...
		<div style="clear: both;"></div>
		<div>
			<h1>No car? Request a ride!</h1>
			<div class="input-group" style="display: block;">
				<span class="input-group-addon-homes"><i class="fa fa-location-arrow fa-fw"></i></span>
				<input class="form-control-homes" type='text' id = 'locFrom_rider' placeholder= "From"/>
			</div>
			<div style="clear: both;"></div>
			<div class="input-group" style="display: block;">
				<span class="input-group-addon-homes"><i class="fa fa-location-arrow fa-fw"></i></span>
				<input class="form-control-homes" type='text' id = 'locTo_rider' placeholder = "To"/>
			</div>
			<div style="clear: both;"></div>
			<div class="input-group" style="display: block;">
				<span class="input-group-addon-homes"><i class="fa fa-calendar-o fa-fw"></i></span>
				<input class="form-control-homes" type="text" id="datepicker_rider" placeholder = "Date" />
			</div>
			<div style="clear: both;"></div>
			<div class = "hours input-group" >
				<span style="position: relative; display: block; margin-left: 37px;"> Preferred Departing Time </span>
				<select class = "times" id='startTimeHour_rider' name="startHour" style="display: inline-block;">
				...
				<span>:</span>
				<select class = "times" id='startTimeMin_rider' name="startMin" style="display: inline-block;">
					<option value="0">00</option>
					<option value="1">01</option>
					<option value="2">02</option>
					<option value="3">03</option>
			...
			<div style="clear:both"></div>
			<div style="clear:both"></div>
			<button class= "submit_button" id="submit_rider" style="margin-left: 7px">SUBMIT REQUEST</button>
			<div style="clear:both"></div>
		</div><!-- div .hero-unit to let drivers create ride offers -->
		<div id="ride_created"></div>
<?php require "static/module/_footer.php"; ?>
</body>
</html>